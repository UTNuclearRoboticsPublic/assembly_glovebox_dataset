...
class LitModel(pl.LightningModule):
    def __init__(self, droprate=0, learning_rate=0.001, test_dropout=False):
        ...
        self.model = UNET(in_channels=3, out_channels=1, droprate=droprate)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # print(f" in training step does raw preds require grad? : {raw_preds.requires_grad}")
    ...
    def validation_step(self, batch, batch_idx):
        x, y = batch
        loss, raw_preds= self._common_set(batch, batch_idx)
        # print(f"raw_preds shape in lightning model val step: {raw_preds.shape}")
        self.log_dict(
            {
                "val_loss": loss,
    ...
    def get_avg_iou(self, raw_preds, y):
        raw_preds = torch.squeeze(torch.sigmoid(raw_preds).ge(0.5).float(),1)

        # print("IN IOU IN IOU IN IOU IN IOU:")
        # print(f"y1 shape: {y[0].shape}")
        # print(f"y2 shape: {y[1].shape}")
    ...
    def test_step(self, batch, batch_idx):
        ...
        def get_avg_ece(raw_preds, y):
            ...
            ece1 = torchmetrics.functional.calibration_error(preds=raw_preds, target=y[0].to(torch.int32), task="binary", num_classes=1)
            ece2 = torchmetrics.functional.calibration_error(preds=raw_preds, target=y[1].to(torch.int32), task="binary", num_classes=1)
    ...
    def get_loss(self, raw_preds, y):
        y1, y2 = y
        # print(f" first in get loss does raw preds require grad? : {raw_preds.requires_grad}")

        # print(f"raw_preds before shape: {raw_preds.shape}")

        raw_preds = torch.squeeze(raw_preds,1) #.requires_grad_()
        # print(f" second in get loss does raw preds require grad? : {raw_preds.requires_grad}")
        # print(f"raw_preds after shape: {raw_preds.shape}")
    ...
    def _common_set(self, batch, batch_idx):
        ...
        raw_preds = self.model(x)
        # print(f"does input require grad? : {x.requires_grad}")
        # print(f" does raw preds require grad? : {raw_preds.requires_grad}")
